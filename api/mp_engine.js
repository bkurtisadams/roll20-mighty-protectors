...
// --- COMPACT TO-HIT BREAKDOWN ---
    ...
    if (prDeducted > 0 || chgDeducted > 0) {
      let costParts = [];
      if (prDeducted > 0) costParts.push(`PR: ${prDeducted} spent`);
      if (chgDeducted > 0) {
    ...
    if (isCrit && critResult) {
      ...
      if (critResult.type === CRIT_TYPES.OFF_BALANCE && !snareHit) html += applyOffBalance(defTok, defName);
      if (critResult.type === CRIT_TYPES.MUSCLE_STRAIN_TARGET && !isVehicleMode(defChar.id) && !snareHit) {
  ...
  // than the engine guessing mook tactics, and areaescape --prone remains
  // available to peel one token off for an individual call.
  function cmdAreaRollNPCs(msg, args) {
    const rollId = args.id;
    const prone = ("prone" in args) ? { prone: "1" } : {};
  ...
  // Force all remaining escapes (GM override). Same --prone batch choice as
  // cmdAreaRollNPCs; this one also sweeps any still-pending player tokens.
  ...
  function cmdMarkerDebug(msg, args) {
    ...
    ids.forEach(tokId => {
      ...
      // Tracked conditions vs. what the token actually stores, so a badge that
